...
class ClassificationsExtractor(BaseFWCExtractor):
    """Extractor for FWC Classifications endpoint.

    Fetches classifications for a specific award from the
    /api/v1/awards/{award_code}/classifications endpoint.
    """
    ...
    async def _fetch_data(
        ...
        """Fetch classifications for an award.
        ...
        if not award_code:
            raise ValueError("award_code is required for classifications extraction")

        async def fetch_page(page: int = 1, limit: int = 100, **kwargs: Any) -> dict[str, Any]:
            return await self.client.get_classifications(
